...
*/
   bus_deliver_event(busno.byte[0], slotno.byte[0], curr);    //first event pass here
}
...
void iodriver_access_complete (int iodriverno, intr_event *intrp)
{
   ...
   if (disksim->traceformat == VALIDATE) 
 {
   ...
   while (req) 
  {
      ...
      req = req->next;
      tmp->next = NULL;
      update_iodriver_statistics();  //nothing

      if ((numreqs = logorg_mapcomplete(sysorgs, numsysorgs, tmp)) == COMPLETE)  // add the statistic time here
     {
...
*/
   ...
   {
      intrp->time += simtime;
      intrp->type = IO_INTERRUPT_COMPLETE;    //generate the 105 event
...
event * iodriver_request (int iodriverno, ioreq_event *curr)
{
   ...
   for (; numreqs>0; numreqs--) 
  {
      ...
      if ((iodrivers[iodriverno]->consttime == IODRIVER_TRACED_QUEUE_TIMES) || (iodrivers[iodriverno]->consttime == IODRIVER_TRACED_BOTH_TIMES))
     {
      ...
      {
         ret->type = IO_ACCESS_ARRIVE;   // in this place IO event changed from 100 to 101
         ret->time = simtime;
...
void iodriver_printstats()
{
   ...
   logorg_printstats(sysorgs, numsysorgs, prefix);  //the source of calculate the average response time 
