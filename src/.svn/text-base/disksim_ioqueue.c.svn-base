...
static void ioqueue_update_arrival_stats (ioqueue *queue, ioreq_event *curr)
{
   ...
   if (queue->printintarrstats) {
      ...
      stat_update(&queue->intarrstats, tdiff);        //disksim_stat.c : 132
      if (curr->flags & READ) {
...
static void ioqueue_insert_new_request (subqueue *queue, iobuf *temp)
{
  ...
  //  ioqueue_print_contents(queue);            //add by menliang
   ...
   */
   ...
   if (tmp->reqcnt == 1) 
   {
	 ...
	 queue->readlen--;
      ...
      // fprintf(stderr, "Updated outtimestats with %f - simtime = %f, iob_un.time = %f\n", simtime - tmp->iob_un.time, simtime, tmp->iob_un.time);
...
*/
   ...
   if (!tmp && ((queue->base.listlen - queue->base.numoutstanding) > 0)) {
      if ((tmp = ioqueue_get_next_request_from_subqueue(&queue->base))) {  //this way 
         queue->lastsubqueue = IOQUEUE_BASE;
      }
   }
   if (tmp) {
...
*/
   ...
   switch(queue->pri_scheme)
   {
      ...
      case ALLEQUAL:
      if ((queue->base.sched_alg == ELEVATOR_LBN)||(queue->base.sched_alg == CYCLE_LBN)||(queue->base.sched_alg == SSTF_LBN)||(queue->base.sched_alg == VSCAN_LBN)) 
	{
           ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno, &tmp->cylinder, &tmp->surface, MAP_NONE);
         }
	 else 
	{
           ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno, &tmp->cylinder, &tmp->surface, -1);  //this way selected
         }

	 ioqueue_insert_new_request(&queue->base, tmp);  // increase the queue->base.listlen
	 ...
	 break;
      ...
      case TWOQUEUE:
        ...
        {
            if ((queue->priority.sched_alg == ELEVATOR_LBN)||(queue->priority.sched_alg == CYCLE_LBN)||(queue->priority.sched_alg == SSTF_LBN)||(queue->priority.sched_alg == VSCAN_LBN)) 
	   {
              ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno, &tmp->cylinder, &tmp->surface, MAP_NONE);
            } else {
...
*/
   ...
   stat_print_set(statset, (3*setsize), sourcestr);  //caculate the Response time average
