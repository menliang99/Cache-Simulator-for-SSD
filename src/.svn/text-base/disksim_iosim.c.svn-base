...
void io_catch_stray_events (ioreq_event *curr)
{
   ...
   case DEVICE_DATA_TRANSFER_COMPLETE:
     ...
     curr->time = simtime + (curr->time * (double) curr->bcount);  //this is the place where 0.0019720 is added to simtime.
...
{
   intrp->type = IO_INTERRUPT_ARRIVE; // 102
   iodriver_interrupt_arrive (0, (intr_event *)intrp);  // disksim_iodriver.c:685
}
...
void io_internal_event(ioreq_event *curr)               //dispatch the ioreq_event
{
   ASSERT(curr != NULL);

  //  if (curr->type != 107)
//{
  // fprintf (stderr, "      Entered io_internal_event       : type %d        time %f (simtime %f)\n", curr->type, curr->time, simtime);
//}

//fprintf (stderr, "@@@@@@@@@Entered io_internal_event, type = %d, internal time = %f, simtime = %f\n", curr->type, curr->time, simtime);

   switch (curr->type) {
   case IO_REQUEST_ARRIVE:    //100 firstly enter this branch
     iodriver_request(0, curr);
     break;

   case IO_ACCESS_ARRIVE:               //included in ssd event
     iodriver_schedule(0, curr);
     break;
   ...
   case IO_INTERRUPT_COMPLETE:                           //included in ssd event 105
     iodriver_interrupt_complete(0, (intr_event *) curr);  //@ disksim_iodriver.c 651
     break;

   case DEVICE_OVERHEAD_COMPLETE:                        //106  included in ssd event

   case DEVICE_ACCESS_COMPLETE:                         //104  included in ssd event
   ...
   case SSD_CLEAN_ELEMENT:                    //301  included in ssd event
   case SSD_CLEAN_GANG:                       //included in ssd event
   case SSD_CACHE_UPDATING:            //  //included in ssd event
     device_event_arrive(curr);            //used in disksim_device.c

     break;

   case BUS_OWNERSHIP_GRANTED:
   case BUS_DELAY_COMPLETE:   //116    later changed to 105 and passed down to ssd_event_arrive   
     bus_event_arrive(curr); //disksim_bus.c : 295
...
event * io_get_next_external_event (FILE *iotracefile)
{
   ...
   temp->type = IO_REQUEST_ARRIVE;//100 just read another event from the file, set the type, but get the time devno blkno bcount flags in iotrace_get_ioreq_event
      
   if (constintarrtime > 0.0) 
      {
	 temp->time = last_request_arrive + constintarrtime;
	 last_request_arrive = temp->time;    //endup time
      }
