...
static void disksim_dumpintq ()
{
   ...
   while (tmp != NULL) {
      i++;
      fprintf (outputfile, "*****menliang*********time %f, simetime is %f, type %d\n", 
	       tmp->time, simtime,  tmp->type);
      fprintf (stderr, "*****menliang*********time %f, simetime is %f, type %d\n", 
	       tmp->time, simtime,  tmp->type);
      tmp = tmp->next;
...
static void disksim_dumpextq ()
{
   ...
   while (tmp != NULL) {
      i++;
      fprintf (outputfile, "########menliang########time %f, type %d\n", 
	       tmp->time, tmp->type);
      fprintf (stderr, "##########menliang###########time %f, type %d\n", 
	       tmp->time, tmp->type);
      tmp = tmp->next;
...
INLINE void addtointq (event *newint)             //frequently used *************************************************
{
  ...
  /*     
         printf(stderr, "remote event: %d %f, local event: %d %f\n", 
 	      tmpevt.type,  
  	      tmpevt.time, 
 	      newint->type,  
 	      newint->time); 
*/
  ...
  //if (newint->type == 107)
 ioreq_event *newint2 = (ioreq_event *) newint;
      //  fprintf(stderr, " Addnewevent: the event is %d, current time is %f, simetime is %f, blkno is %d\n",  newint->type, newint->time, simtime, newint2->blkno);
 // if (newint->type == 105)
    //    fprintf(stderr, " Addnewevent: the event is %d, current time is %f, simetime is %f\n",  newint->type, newint->time, simtime);
...
INLINE static event * getfromintq ()              //from here get the event and pass down.
{
   event *temp = NULL;
...
fprintf(stderr, "\nThe total cache write number is %d.\n", cache_total_write_count);
fprintf(stderr, "The total Flash write number is %d.\n\n", ssd_total_write_count);
fprintf(stderr, "The total cache read number is %d.\n", cache_total_read_count);
fprintf(stderr, "The total Flash read number is %d.\n\n", ssd_total_read_count);

fprintf(stderr, "The total cache clean number is %d.\n", cache_total_clean_count);

fprintf(stderr, "The total Flash clean number is %d.\n", ssd_total_clean_count);
...
static void initialize ()
{
   ...
   if (disksim->external_control | disksim->synthgen | disksim->iotrace) {
      io_initialize(val);                                                          //disksim_iosim.c
   }
   if (disksim->synthgen) {
...
INLINE static event * 
getnextevent(void)                          // get next event either from the iotracefile or someother place.
{
  ...
  //if (simtime != curr->time)
   // if (curr->type == 107) 
    ioreq_event *curr2 = (ioreq_event *) curr;
  // fprintf(stderr, " *****************************Getnextevent: the event is %d, current time is %f, simetime is %f, blknois %d\n",  curr->type, curr->time, simtime, curr2->blkno);

   // if (curr->type == 105)  
  // fprintf(stderr, " Getnextevent: the event is %d, current time is %f, simetime is %f\n",  curr->type, curr->time, simtime);
     simtime = curr->time;             //IMPORTANT: simtime is updated at this point.
                                        //first check the intq, if there is a envnt then return, else get an event from the outside tracefile.
      if (curr->type == NULL_EVENT)       //0
       {          
...
//input pararmters argv[0]----argv[5]
void disksim_setup_disksim (int argc, char **argv)
{
  ...
  fprintf (outputfile, "*** I/O trace used: %s\n", argv[4]);           //argv[4] is the filename of the trace file
  fflush (outputfile); 
...
/*    } */
  ...
  if(disksim_loadparams(argv[1], disksim->synthgen)) {                              //stepi to see how to load params
...
void disksim_run_simulation ()
{
  ...
  while (disksim->stop_sim == FALSE) {
   ...
   curr = disksim_simulate_event(event_count);
 //  disksim_dumpintq();
//disksim_dumpextq ();
    event_count++;
  }
...
event  *disksim_simulate_event (int num)
{
  event *curr;
  ...
  if (simtime != curr->time)
   ...
   fprintf(stderr, "Eventdispatch: the event is %d, current time is %f, simetime is %f\n",  curr->type, curr->time, simtime);

    simtime = curr->time; 

    
    if (curr->type == INTR_EVENT)    //98 changed to 102 later
    {
...
//************************************************************************
   ...
   ssd_total_write_count = 0;
   cache_total_write_count = 0;
   ssd_total_read_count = 0;
   cache_total_read_count = 0;
   ssd_total_clean_count = 0;
   cache_total_clean_count = 0;
   ssd_current_idle_time = 0.0;
//**************************************************************menliang
