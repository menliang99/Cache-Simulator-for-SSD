...
void bus_delay(int busno, int devtype, int devno, double delay, ioreq_event *curr)
{
 ...
 //  fprintf (stderr, "bus_DELAY_enter,- devno %d, type %d, internal time %f, simtiem = %f\n*********\n", tmp->devno, tmp->type, tmp->time, simtime);
   tmp->type = BUS_DELAY_COMPLETE;
   tmp->time = simtime + delay;
   tmp->devno = devno;
   tmp->busno = busno;
   tmp->devtype = devtype;
   tmp->delayed_event = curr; //temporarily store the current event. release and dispatch it in the bus_event_arrive
   addtointq((event *) tmp);
//  fprintf (stderr, "bus_DELAY_leave,- devno %d, type %d, internal time %f, simtiem = %f\n*********\n", tmp->devno, tmp->type, tmp->time, simtime);
}


void bus_event_arrive(ioreq_event *ptr)
{
   bus_event *curr = (bus_event *) ptr;  //how to change? generated the delayed_event in ptr
   ...
   //fprintf (stderr, "bus_event_arrive,event-type has changed form %d to %d,  internal time %f, simtiem = %f\n*********\n", curr->type, curr->delayed_event->type, curr->time, simtime);
  
    switch (curr->devtype) {

      case CONTROLLER: //5
         if (curr->type == BUS_DELAY_COMPLETE) {
	    controller_bus_delay_complete (curr->devno, curr->delayed_event, curr->busno); //curr->delayed_event is changed to 101,102,105,109.
         } else {
	 ...
	 break;
      ...
      case DEVICE:     //4
         if (curr->type == BUS_DELAY_COMPLETE) {
            //fprintf (stderr, "bus_event_arrive,-type %d,  internal time %f, simtiem = %f\n*********\n", curr->type, curr->time, simtime);
...
*/
//fprintf (stderr, "!!!!!!!!!!Entered bus deliever_event, type = %d, devtype = %d, simtime = %f\n", curr->type, currbus->slots[slotno].devtype, simtime);
   devno = currbus->slots[slotno].devno;
   switch (currbus->slots[slotno].devtype) {
   ...
   device_event_arrive(curr);                       //passed event down to the ssd_event_arrive
