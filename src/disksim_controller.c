...
*/
   ...
   if (busno == controller_get_upward_busno(currctlr, curr, &slotno)) 
  {
      ...
      if (busno == -1) 
      {
         intr_request ((event *)curr);  //create the event 98
      } else
      {
   ...
   {
      buswaitdir = DOWN;
      bus_deliver_event(busno, slotno, curr); //this way
      if (currctlr->outbusowned != -1) {
         currctlr->outbusowned = -1;
      }
   } else {
...
void controller_event_arrive (int ctlno, ioreq_event *curr)          // bus_deliever_event in disksim_bus.c
{
   controller *currctlr = getctlr(ctlno);
   ...
   */
   ...
   switch (currctlr->type) {
      ...
      case CTLR_PASSTHRU:                 //deal with the first event of 120 
