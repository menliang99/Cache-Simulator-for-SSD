...
void controller_passthru_event_arrive (controller *currctlr, ioreq_event *curr)            //deal with the first event in starting up event = 120
{
   int busno;
   int slotno;
...
*/
	 ...
	 break;
      ...
      case IO_QLEN_MAXCHECK:                         //120  down from controller_event_arrive, in disksim _controller.c
	 busno = controller_get_downward_busno(currctlr, curr, &slotno);
	 bus_deliver_event(busno, slotno, curr);
	 break;
