...
void iotrace_set_format (char *formatname)
{
   ...
   disksim->traceendian = _LITTLE_ENDIAN;          //add new format in disksim_global.h
   ...
   } else if (strcmp(formatname, "spc") == 0) {
        /* new tracefile downloaded from Financial--added by menliang*/
      disksim->traceformat = SPC;
   } else if (strcmp(formatname, "msrc") == 0) {
        /* new tracefile downloaded from microsoft--added by menliang*/
      disksim->traceformat = MSRC;
   } else {
...
static ioreq_event * iotrace_ascii_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   ...
   if (new->flags & ASYNCHRONOUS) {                   //ASYNCHRONOUS == 0x00000100
      new->flags |= (new->flags & READ) ? TIME_LIMITED : 0;
   } else if (new->flags & SYNCHRONOUS) {             //SYNCHRONOUS == 0x00000080    in disksim_reflags.h and global.h
      new->flags |= TIME_CRITICAL;
   }
...
//******************************************************************************************************************
static ioreq_event * iotrace_SPC_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   char line[201];
   char operation[1];
   int i;
   ...
   if (fgets(line, 200, tracefile) == NULL) {
    ...
    i = sscanf(line, "%d,%d,%d,%c,%lf\n", &new->devno, &new->blkno, &new->bcount, operation, &new->time); //operation cannot be %s
   ...
   new->blkno = new->blkno / 512;
   new->bcount = new->bcount / 512;
   new->buf = 0;
   new->opid = 0;
   new->busno = 0;
   new->cause = 0;
...
static ioreq_event * iotrace_MSRC_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   ...
   if (fgets(line, 300, tracefile) == NULL) {
    ...
    i = sscanf(line, "%lf %s %d %s %d %d %d\n", &new->time, device, &new->devno, operation, &new->blkno, &new->bcount, &response_time); //operation cannot be %s
   ...
   new->time = new->time / 10000;
//the windows filetime is based on the unit of 100ns. we need to change it to the unit of ms.
   new->bcount = new->bcount / 512;
// Implemented in SSD simulation. Each operation is based on one page, the size of which is 4KB, and is devided into 8 lines. So each line is 512 bytes.
   new->blkno = new->blkno / 512;
   new->buf = 0;
   new->opid = 0;
   new->busno = 0;
...
ioreq_event * iotrace_get_ioreq_event (FILE *tracefile, int traceformat, ioreq_event *temp)
{
   ...
   //***added by menliang*********
   case SPC:     //getting from 
       temp = iotrace_SPC_get_ioreq_event(tracefile, temp);
       break;
   case MSRC:    //getting from microsoft
       temp = iotrace_MSRC_get_ioreq_event(tracefile, temp);
       break;
