...
static void logorg_maprequest_update_stats (logorg *currlogorg, ioreq_event *curr, outstand *req, int numreqs)
{
  ...
  // fprintf (stderr, "Entering logorg_maprequest_update_stats - numreqs %d, orgdevno %d\n", numreqs, orgdevno);
...
*/
   ...
   {
      fprintf(stderr,"Stopping simulation because of saturation: simtime %f, totalreqs %d, QueueLength is %d\n", simtime, disksim->totalreqs, currlogorg->stat.outstanding);
...
static void logorg_mapcomplete_update_stats (logorg *currlogorg, ioreq_event *curr, outstand *req)
{
 ...
 float p = simtime - req->arrtime;
   //fprintf(stderr, "\033[47;31m$$$$$$$$$statistic the resptimestats is in Event 105. the arrive time is %f the time interval is %f. the opid is %d. \033[0m\n", req->arrtime, p, req->opid);
...
static void logorg_addnewtooutstandq (logorg *currlogorg, outstand *temp)   //add into the outstand queue at the Event 0
{
   outstand *run = NULL;

   temp->next = NULL;
   run = currlogorg->hashoutstand[(temp->opid % HASH_OUTSTAND)];  //HASH_OUTSTAND   20
   if (run == NULL) {
      currlogorg->hashoutstand[(temp->opid % HASH_OUTSTAND)] = temp;  //hash table algorithm
   } else {
      while (run->next != NULL)
         run = run->next;
      run->next = temp;
...
//fprintf (outputfile, "flags %x, opid %d\n", curr->flags, curr->opid);
   ...
   ASSERT1((numlogorgs >= 1) && (numlogorgs < MAXLOGORGS), "numlogorgs", numlogorgs);   //maxlogorgs == 100
   ...
   for (i = 0; i < numlogorgs; i++)
 {
     // i.e. array
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
     while ((curr->blkno + curr->bcount) >= (logorgs[i]->blksperpart * logorgs[i]->numdisks))
        {
	 curr->blkno = curr->blkno / 8 ;
         while ( (curr->bcount) >= (logorgs[i]->blksperpart * logorgs[i]->numdisks))
              {
	       curr->bcount = curr->bcount - 8 ;
             }
        }
/////&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
//added by menliang, for testing and adopting all the address
   ...
   if (maptype == ASIS) {
      ...
      numreqs = logorg_mapstriped(logorgs[logorgno], curr);  //this way selected
   } else {
...
static void logorg_printreqtimestats (logorg *currlogorg, char *prefix)
{
  ...
  //  stat_print(&currlogorg->stat.acctimestats, prefix);
}
...
void logorg_printstats (logorg **logorgs, int numlogorgs, char *sourcestr)
{
   ...
   for (i = 0; i < numlogorgs; i++) {
      ...
      logorg_printreqtimestats(logorgs[i], prefix);  //to statistic the average response time
