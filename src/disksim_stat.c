...
/*
 if ((statptr->runval >= 0.2126) &&  (statptr->runval <= 0.26))
  {
  fprintf(stderr, "In stat_update, the accumulated time is %f\n",statptr->runval);
  }
*/
   if (buckets > DISTSIZE) {
      if (intval < start) {
...
void stat_print_file (statgen *statptr, char *identstr, FILE *outfile){
   ...
   fprintf(stderr, "menliang add: The total time is %f, the total count is %d\n", statptr->runval, statptr->count);
