...
double ssd_get_blktranstime (ioreq_event *curr)
{
   ...
   tmptime = bus_get_transfer_time(ssd_get_busno(curr), 1, (curr->flags & READ));  //0.0002384 * 8 = 0.0019720
...
static void ssd_send_event_up_path (ioreq_event *curr, double delay)
{
   ...
   // fprintf (stderr, "ssd_send_event_up_path - devno %d, type %d, cause %d, blkno %d, simtiem = %f\n\n\n", curr->devno, curr->type, curr->cause, curr->blkno, simtime);
   ...
   curr->next = currdisk->buswait;    //buswait is also a type of ioreq_event
 ...
 if (currdisk->busowned == -1) 
  {
   ...
   if (currdisk->busowned == busno)
     {       //busno = 1;
      //fprintf (outputfile, "Already own bus - so send it on up\n");
      bus_delay(busno, DEVICE, curr->devno, delay, curr); //if there is no bus_delay, most event will not happen, create event116 and addtointq
     } else { 
...
static void ssd_check_channel_activity (ssd_t *currdisk)
{
 ...
 while (1) 
 {
       ...
       if (curr != NULL) 
     {
           ...
           if (currdisk->neverdisconnect)                 /* already connected */
         {           
               if (curr->flags & READ) 
               {
...
void ssd_bus_delay_complete (int devno, ioreq_event *curr, int sentbusno) //comes from device_bus_delay_complete, which is in disksim_device.c and mapped in to
 									     // disksim_iosim.c. ioreq_event == 116, BUS_DELAY_COMPLETE
{
   ssd_t *currdisk;
   intchar slotno;
   intchar busno;
   ...
   //fprintf (stderr, "!!!!!!!!!!Entered ssd_bus_delay_complete\n");
...
static void ssd_request_complete(ioreq_event *curr)
{
 ...
 //  fprintf(stderr, "\033[47;33mssd_request_complete time is %f, the physical access done type is %d\033[0m\n", currdisk->queue->base.accstats.runval, x->type);
 ...
 */
unsigned long ssd_logical_pageno(unsigned long blkno, ssd_t *s)
{
    unsigned long apn;
    unsigned long lpn;
  ...
  lpn = ((apn - (apn % (s->params.element_stride_pages * s->params.nelements)))/s->params.nelements) + (apn % s->params.element_stride_pages);  //... + 0
  //lpn = ( apn - apn % 8 ) / 8 + apn % 1;
  //actually it is lpn  = apn / 8; the purpose is finding the place that apn in its place
   // fprintf(stderr, "the blkno is %d, the lpn is %d...\n", blkno, lpn);
    return lpn;
}
...
{
    double clean_cost = ssd_clean_element(s, elem_num);  // in ssd_clean.c line 1007
    return clean_cost;
}
...
static int ssd_invoke_element_cleaning(int elem_num, ssd_t *s)
{
    ...
    ASSERT(elem->media_busy == FALSE);      // element must be free

    max_cost = _ssd_invoke_element_cleaning(elem_num, s);

    // cleaning was invoked on this element. we can start the next operation on this elem only after the cleaning gets over.

    if (max_cost > 0) {     // invoke cleaning when the number of free blocks drop below a certain threshold (in a plane or an element).

        ioreq_event *tmp;
        elem->media_busy = 1;
        cleaning_invoked = 1;

        // we use the 'blkno' field to store the element number, which leads to confusion.
...
fprintf(stderr, "cleaning started in SSD.c, cost is %f\n", max_cost);
...
///$$$$$$$$$$$$$$$$$$$$$$%%%%%%%%%%%%SSD_ACTIVATE_ELEM%%%%%%%%%%%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
void ssd_activate_elem(ssd_t *currdisk, int elem_num)             //seems always run
{
    ioreq_event *req;
    ssd_req **read_reqs;
    ssd_req **write_reqs;
    ...
    if (elem-> media_busy == TRUE) {               //parallel dealing with the multiple writing.
        return;
    } 

    ASSERT(ioqueue_get_reqoutstanding(elem->queue) == 0);  // we can invoke cleaning in the background whether there is request waiting or not
    
/*
    if (currdisk->params.cleaning_in_background)   // if cleaning was invoked, wait until it is over ...
      { 
...
*/
    ...
    if (elem->metadata.reqs_waiting > 0)    
  {
       ...
       /* if (!currdisk->params.cleaning_in_background)   // //enter into element_cleaning // invoke cleaning in foreground when there are requests waiting
         {          
               if (ssd_invoke_element_cleaning(elem_num, currdisk)) 
               {  
  //fprintf(stderr, "\t\t\t\t cleaning started in SSD.c  \n");
                return;
               }
          }*/
          //delete SSD_ELEMENT_CLEAN event from the flow. element cleaning only excutes when it is the last page in less 5% block  --menliang
        
        if (currdisk->params.copy_back == SSD_COPY_BACK_DISABLE)   // how many reqs can we issue at once
          {
            max_reqs = 1;            // it is the always true fact.
          } 
         else 
          {
		...
		//in order to simplify the overlapping strategy,let's issue requests of the same type together.
		//**********************************************************************************************
        read_reqs = (ssd_req **) malloc(max_reqs * sizeof(ssd_req *));
        write_reqs = (ssd_req **) malloc(max_reqs * sizeof(ssd_req *));


        while ((req = ioqueue_get_next_request(elem->queue)) != NULL)           // collect the requests
         {
	    int found = 0;
            elem->metadata.reqs_waiting --;

		// see if we already have the same request in the list. this usually doesn't happen -- but on synthetic traces this weird case can occur.
            if (req->flags & READ)    
            ...
            if (!found) 
           {
                ...
                if ((read_total >= max_reqs) || (write_total >= max_reqs))
                    break;
            }
             else 
            {
        ...
        if (read_total > 0) 
        {
            ...
            for (i = 0; i < read_total; i ++)    // add an event for each request completion
	    {
              ...
              if (schtime < read_reqs[i]->schtime)        // find the maximum time taken by a request
		{
              ...
              stat_update (&currdisk->stat.acctimestats, read_reqs[i]->acctime);  //time accumulating, --menliang
          ...
          //printf("R: blk %d elem %d acctime %f simtime %f\n", read_reqs[i]->blk, elem_num, read_reqs[i]->acctime, read_reqs[i]->org_req->time);
         //fprintf(stderr, "read finished, the accumulated time is %f\n", currdisk->stat.acctimestats.runval);
        ...
        if (write_total > 0)                //iotracefile -->flag = 0   write_total always equals 1
         {   
            ssd_compute_access_time(currdisk, elem_num, write_reqs, write_total); // next issue the write requests
            ...
            for (i = 0; i < write_total; i ++)
           {
              ...
              //simtime = write_reqs[i]->org_req->time;
              //printf("blk %d elem %d acc time %f\n", write_reqs[i]->blk, elem_num, write_reqs[i]->acctime);
        // fprintf(stderr, "write finished, the accumulated time is %f\n", currdisk->stat.acctimestats.runval);
              ...
              //printf("W: blk %d elem %d acctime %f simtime %f\n", write_reqs[i]->blk, elem_num, write_reqs[i]->acctime, write_reqs[i]->org_req->time);
...
{
   ssd_t *currdisk = getssd(curr->devno);
   unsigned long blkno = curr->blkno;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   while (count != 0) {                           //The block number in one tracfile line reduce 8 every time, until reach 0.                
       ...
       tmp->devno = curr->devno;     //0
       tmp->busno = curr->busno;    //256
       tmp->flags = curr->flags;
       tmp->blkno = blkno;
       tmp->bcount = ssd_choose_aligned_count(currdisk->params.page_size, blkno, count);          //always<=8
     
        //ASSERT(tmp->bcount == currdisk->params.page_size);           //make sure operates on 8 blocks one time. removed by menliang
       ...
       ioqueue_add_new_request(elem->queue, (ioreq_event *)tmp);  //un-finished events are temporarily stored in ioqueue, finished addtointq
...
static void ssd_media_access_request (ioreq_event *curr)
{
    ...
    switch(currdisk->params.alloc_pool_logic) {
        ...
        case SSD_ALLOC_POOL_CHIP:  //   1  -- each element is an allocation pool
...
static void ssd_reconnect_done (ioreq_event *curr)
{
   ...
   if (curr->flags & READ) 
  {
      if (currdisk->neverdisconnect) 
      {
         /* Just holding on to bus; data transfer will be initiated when media access is complete.                                    */
         addtoextraq((event *) curr);
         ssd_check_channel_activity (currdisk);
      } 
      else 
      {
         /* data transfer: curr->bcount, which is still set to original requested value, indicates how many blks to transfer.       */
         curr->type = DEVICE_DATA_TRANSFER_COMPLETE;
         ssd_send_event_up_path(curr, (double) 0.0);
      }
   ...
   else 
  {
      if (currdisk->reconnect_reason == DEVICE_ACCESS_COMPLETE)
     {
      ...
      {            //this is the way selected---menliang the reason is what?
         /* data transfer: curr->bcount, which is still set to original requested value, indicates how many blks to transfer.       */
         curr->type = DEVICE_DATA_TRANSFER_COMPLETE;  //109
         ssd_send_event_up_path(curr, (double) 0.0);
      }
   }
}
...
static void ssd_request_arrive (ioreq_event *curr)
{
  ...
  // ioqueue_add_new_request(currdisk->queue, curr);
   ...
   if (currdisk->channel_activity == NULL) {  //channel_activity is a type of ioreq_event

      curr = ioqueue_get_next_request(currdisk->queue);    //sometimes it is a deadlock
...
static void ssd_clean_element_complete(ioreq_event *curr)
{
   ...
   addtoextraq((event *) curr);     // release this event
   currdisk->elements[elem_num].media_busy = 0;     // activate the gang to serve the next set of requests
   ssd_activate_elem(currdisk, elem_num);
      // printf("ssd_clean_element_complete  ...\n");
}
...
void ssd_complete_parent(ioreq_event *curr, ssd_t *currdisk)
{
    ...
    if (parent->tempint2 == 0) {
      ...
      if (currdisk->channel_activity == NULL) {
         ssd_check_channel_activity (currdisk);  //run to statistic the time
      }
    }
}
...
static void ssd_access_complete_element(ioreq_event *curr)
{
   ...
   if ((x = ioqueue_physical_access_done(elem->queue,curr)) == NULL) {  //remove the 107 event from the queue.base
   ...
   addtoextraq((event *) curr);  //remove the 107 event from the intq
   ssd_activate_elem(currdisk, elem_num);
   //ssd_activate_cache_updating(currdisk, elem_num); //invoke_cache_updating is moved into ssd_activate_elem
    // printf("ssd_access_complete_element   ...\n");
}
...
static void ssd_access_complete(ioreq_event *curr)
{
    ...
    switch(currdisk->params.alloc_pool_logic) {
        ...
        case SSD_ALLOC_POOL_CHIP:
            ssd_access_complete_element(curr);      //this way selected
        break;
...
static void ssd_interrupt_complete (ioreq_event *curr)
{
   ...
   switch (curr->cause) {
      ...
      case RECONNECT:   //-2
         ssd_reconnect_done(curr);  //this way selected
     break;
...
void ssd_event_arrive (ioreq_event *curr)   //How does the event pass down to here?? through the function of device_event_arrive in disksim_device.c
{
   ssd_t *currdisk;

  //fprintf (outputfile, " - devno %d, blkno %d, type %d, simtime = %f\n", curr->devno, curr->blkno, curr->type, simtime);
   currdisk = getssd (curr->devno);

  // fprintf (stderr, "Entered  ssd_event_arrive, ioreq type %d (simtime %f)\n", curr->type, simtime);
   switch (curr->type) {   //disksim_iosim.h

      case IO_ACCESS_ARRIVE:          //101*******happens when load one line in trace file 
 	
       	// curr->time = simtime + currdisk->overhead;
         //curr->type = DEVICE_OVERHEAD_COMPLETE;

         curr->time = simtime;
         curr->type = SSD_CACHE_UPDATING;
         addtointq((event *) curr);
         break;

      case DEVICE_OVERHEAD_COMPLETE:   //106
     //fprintf (stderr, "Entered  DEVICE_OVERHEAD_COMPLETE: time %f (simtime %f)\n", curr->time, simtime);
         ssd_request_arrive(curr);
         break;

      case DEVICE_ACCESS_COMPLETE:     //107**most case**//come when write or read continuously

         ssd_access_complete (curr);   //include ssd_access_complete_element
         break;
      ...
      case IO_QLEN_MAXCHECK:   //120       appear at the very begin of simtime, didn't get data from tracefile yet.
      ...
      case SSD_CLEAN_GANG:
        fprintf (stderr, "Entered SSD_CLEAN_GANG: time %f (simtime %f)\n", curr->time, simtime);
          ssd_clean_gang_complete(curr);
          break;

      case SSD_CLEAN_ELEMENT:            // it is active after plenty of tracefiles
          // fprintf (stderr, "Entered SSD_CLEAN_ELEMENT: time %f (simtime %f)\n", curr->time, simtime);
          ssd_clean_element_complete(curr);
          break;

//##############################################################################################
      case SSD_CACHE_UPDATING:  //303,just an event, not included in the queue and scheduled.
      	 ssd_cache_updating_complete(curr); 
       	 curr->time = simtime + currdisk->overhead;
         curr->type = DEVICE_OVERHEAD_COMPLETE;
         addtointq((event *) curr);
      	  break;
...
void ssd_get_mapping (int maptype, int devno, unsigned long blkno, int *cylptr, int *surfaceptr, int *blkptr)
{
   ssd_t *currdisk = getssd (devno);

   if ((blkno < 0) || (blkno >= currdisk->numblocks)) {            //numblocks = 16384 (16K) * 4096 = 67108864 = 2^26 =  64M
...
{
//upper functions: lp_override_inst, lp_instantiate, lp_inst_list, libparampars, lp_loadfile, disksim_loadparams, disksim_setup_disksim
...
void ssd_printsetstats (int *set, int setsize, char *sourcestr)                  ///SSD outputfile main statistics function
{
   int i;
...
void ssd_printstats (void)
{
   ...
   if (reqcnt == 0) {
      fprintf(outputfile, "No ssd requests encountered\n");     //like the simpledisk and disk statistics illustrated
      return;
   }

   ioqueue_printstats(queueset, queuecnt, prefix);            //disksim/src/ioqueue.c
   ...
   fprintf (outputfile, "+++++++++++++++++++++added+++++++by++++++++++menliang+++++++++++++++++\n\n");
