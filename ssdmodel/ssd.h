...
typedef struct _page_metadata{
    ...
    float      write_count;                // state of the page  ---added by menliang
                                        // a page could be in one of the following states:
                                        // EMPTY - when the page has not been written
                                        // CACHED - when the valid data of the page is in the cache
                                        // INSSD - when the valid data of the page is in the Flash
...
typedef struct _ssd_element {
   struct ioq *queue;
   int media_busy;
   double cache_updating_time;    //added by menliang
   double last_access_time;       //added by menliang, oct2011
...
#define SSD_DATA_BYTES_PER_SECTOR               512   //each line has 512 bytes
...
typedef struct _ssd_timing_params {
    ...
    int    nelements;                   // how many concurrent entities (chips)    8
    int    page_size;                   // sectors per page           8
    int    pages_per_block;             // pages per block            64
    int    blocks_per_element;          // total blocks per chip      16384
    int    element_stride_pages;        // usually 1 or 2             1
    ...
    double chip_xfer_latency;           // time to get a byte to/from the chip register       2.5e-5
    double page_read_latency;           // time to read a page into chip register         0.025
    double page_write_latency;          // time to write a page from chip register          0.2
    double block_erase_latency;         // time to erase a block                          1.5

    int     write_policy;               // policy followed when writing a block           2
                                        // follow the above definitions
                                        // (e.g., DISKSIM_SSD_WRITE_POLICY_SIMPLE)

    int     reserve_blocks;             // percentage of blocks to reserve            15

    int     min_freeblks_percent;       // min free blocks percentage                 5

    int     cleaning_policy;            // cleaning & wear-leveling policy used to    2
                                        // clean blocks

    int     planes_per_pkg;             // num of planes in a single SSD package      8

    unsigned int    blocks_per_plane;   // num of flash blocks in a single plane      2048

    int     plane_block_mapping;        // how blocks are mapped in a plane         3

    int     copy_back;      // how active pages are assigned in an element       0

    int     num_parunits;               // number of parallel units in each element   1

    int     elements_per_gang;          // number of packages in a gang      1

    int     gang_share;                 // is this a shared bus or shared control gang?      1

    int     cleaning_in_background;     // do we want to do the cleaning in foreground/background?    0

    int     alloc_pool_logic;           // static or dynamic allocation     1
} ssd_timing_params;
...
typedef struct ssd {
   ...
   double overhead;
   double bus_transaction_latency;      //0
   int numblocks;
   int devno;
   int inited;
   ...
   int cache_updating_in_progress;
   int cache_updating_elem_num;
...
void    ssd_activate_elem(ssd_t *currdisk, int elem_num); //used in cache.c
