...
// if the write is within the same plane, then the data need not cross the pins. but if not, add the cost of transferring the bytes across the pins
    ...
    return cost;
   fprintf(stderr, "ssd write cost is %f, ssd clean element cost is\n", cost);
}
...
static int ssd_pick_block_to_clean2(int plane_num, int elem_num, double *mcost, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    *mcost = 0;
    ...
    for (i = 0; i < s->params.blocks_per_element; i ++) {    //s->params.blocks_per_element==16384
...
static double _ssd_clean_block_partially(int plane_num, int elem_num, ssd_t *s)
{
  ...
  //fprintf(stderr, "\t\t\t\tthe cleaning block number is %d, valid page in the block is %d\n", block,metadata->block_usage[block].num_valid );
    if (metadata->block_usage[block].num_valid > 0)  //s->params.pages_per_block==64
     {          
        for (i = 0; i < s->params.pages_per_block; i ++) 
        {   
            int lp_num = metadata->block_usage[block].page[i];    // pick a page that is not yet cleaned and move it

            if (lp_num != -1) //invalid page is -1
	    {
 // fprintf(stderr, "\t\t\t\tthe cleaning block number is %d, valid page in the block is %d\n", block,metadata->block_usage[block].num_valid );
                cost += ssd_clean_one_page(lp_num, i, block, plane_num, elem_num, metadata, s);
                break;  //break from where?
            }
    ...
    if (metadata->block_usage[block].num_valid == 0) 
     {   // if we've moved all the valid pages out of this block, then we're done with it. so, erase it and update the system state.
    
        cost += s->params.block_erase_latency ;  // add the cost of erasing the block   //s->params.block_erase_latency = 1.5
...
double _ssd_clean_block_fully(int blk, int plane_num, int elem_num, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    return cost;   //1.5  just erase the whole block, no valid page and no page transfer
}
...
/*
 * first we create a hash table of blocks according to their usage. then we select blocks with the least usage and clean them.
 */
...
static double ssd_clean_blocks_greedy(int plane_num, int elem_num, ssd_t *s)
{
    ...
    /////////////////////////////////////////////////////////////////////////////
// we now have a hash table of blocks, where the key of each bucket is the usage count and each bucket has all the blocks with the same usage count (i.e., the same num of valid pages).


 for (i = 0; i <= s->params.pages_per_block; i ++)    //params.pages_per_block == 64    
   {      
        int j;
        usage_table *entry;       
        entry = &(table[i]);    // get the bucket of blocks with 'i' valid pages    

        for (j = 0; j < entry->len; j ++)   // free all the blocks with 'i' valid pages
       {
            int blk = entry->block[j];
            int block_life = metadata->block_usage[blk].rem_lifetime;
            
            if ((plane_num != -1) && (metadata->block_usage[blk].plane_num != plane_num)) 
               {                 // if this is plane specific cleaning, then skip all the blocks that don't belong to this plane.
                continue;
               }
            if (block_life == 0)
               {               // if the block is already dead, skip it
            ...
            if (ssd_can_clean_block(s, metadata, blk))  // clean only those blocks that are sealed.
              {               
                if (s->params.cleaning_policy == DISKSIM_SSD_CLEANING_POLICY_GREEDY_WEAR_AWARE)   //escaped this way	
                 {    // if we care about wear-leveling, then we must rate limit overly cleaned blocks
		     //see if this block's remaining lifetime is within a certain threshold of the average remaining lifetime of all blocks in this element
                    if (block_life < (SSD_LIFETIME_THRESHOLD_X * avg_lifetime)) 
                    {       // we have to rate limit this block as it has exceeded its cleaning limit
                        printf("Rate limiting block %d (block life %d avg life %f\n",blk, block_life, avg_lifetime);
                        if (ssd_rate_limit(block_life, avg_lifetime)) 
                       {                                          // skip this block and go to the next one
                            continue;
                ...
                ssd_total_clean_count++;
                     // okies, finally here we're with the block to be cleaned invoke cleaning until we reach the high watermark.
    ...
    if (!ssd_stop_cleaning(plane_num, elem_num, s)) {      // see if we were able to generate enough free blocks
...
double ssd_clean_element_no_copyback(int elem_num, ssd_t *s)
{
    ...
    switch(s->params.cleaning_policy) {
        ...
        case DISKSIM_SSD_CLEANING_POLICY_GREEDY_WEAR_AWARE:
            cost = ssd_clean_blocks_greedy(-1, elem_num, s);  //selected this way
            break;
...
double ssd_clean_element(ssd_t *s, int elem_num)
{
    ...
    if (s->params.copy_back == SSD_COPY_BACK_DISABLE) {
        cost = ssd_clean_element_no_copyback(elem_num, s);    //this way selected
    } else {
 ...
 * certain threshold (in a plane or an element).bre
 */
int ssd_start_cleaning(int plane_num, int elem_num, ssd_t *s)
{
    if (plane_num == -1) {
        unsigned int low = (unsigned int)LOW_WATERMARK_PER_ELEMENT(s);    //low = 819
        return (s->elements[elem_num].metadata.tot_free_blocks <= low);     //total 2456
    } else {
...
int ssd_stop_cleaning(int plane_num, int elem_num, ssd_t *s)
{
    if (plane_num == -1) {
        ...
        return (s->elements[elem_num].metadata.tot_free_blocks > high);        //    high = 820
    } else {
