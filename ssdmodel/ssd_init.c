...
void ssd_element_metadata_init(int elem_number, ssd_element_metadata *metadata, ssd_t *currdisk)
{
    ...
    unsigned int tot_blocks = currdisk->params.blocks_per_element;              //16384
    unsigned int tot_pages = tot_blocks * currdisk->params.pages_per_block;     //1048576
    ...
    unsigned int bsn = 1;
    int plane_block_mapping = currdisk->params.plane_block_mapping;            //3

    //////////////////////////////////////////////////////////////////////////////
    // active page starts at the 1st page on the reserved section
    reserved_blocks_per_plane = (currdisk->params.reserve_blocks * currdisk->params.blocks_per_plane) / 100;       //512 = 25*64/100
    usable_blocks_per_plane = currdisk->params.blocks_per_plane - reserved_blocks_per_plane;        //  1536 = 2048 -512
    reserved_blocks = reserved_blocks_per_plane * currdisk->params.planes_per_pkg;                 // 512*8 = 4096
    usable_blocks = usable_blocks_per_plane * currdisk->params.planes_per_pkg;                     //  12288
    ...
    for (i = 0; i < (unsigned int)currdisk->params.planes_per_pkg; i ++) {              //8
        int blocks_to_skip;
        ...
        switch(plane_block_mapping) {
            ...
            case PLANE_BLOCKS_FULL_STRIPE:                                              //This way selected
    ...
    switch(plane_block_mapping) {
        ...
        case PLANE_BLOCKS_FULL_STRIPE:  // this way selected, active_page == 891392 = 8 * 1741 * 64
    ...
    export_size = usable_blocks * SSD_DATA_PAGES_PER_BLOCK(currdisk);      // the maximum lpn that can be used. if lpn excess export_size, ERROR happens.
    currdisk->data_pages_per_elem = export_size;                           //export_size = 877464
    ...
    // what if the no of blocks is not divisible by 8?  WHY????WHY???
...
//********************************************************************************* 
    //////////////////////////////////////////////////////////////////////////////
    // allocate the page usage array and initialize it
    if (!(metadata->page_usage = (page_metadata *)malloc(tot_pages * sizeof(page_metadata)))) {
        fprintf(stderr, "Error: malloc to page_usage in ssd_element_metadata_init failed\n");
        fprintf(stderr, "Allocation size = %d\n", tot_pages * sizeof(page_metadata));
        exit(1);
    }
    bzero(metadata->page_usage, tot_pages * sizeof(page_metadata));

   for(i = 0; i < tot_pages; i++){
 
       metadata->page_usage[i].write_count = 0; 

   }
//********************************************************************************8menliang */
    //////////////////////////////////////////////////////////////////////////////
    // initially, we assume that every logical page is mapped onto a physical page. we start from the first phy page and continue to map, leaving the last page of every block to store the summary information.
    ppage = 0;
    i = 0;
    while (i < export_size) 
  {
        ...
        if (ssd_last_page_in_block(ppage, currdisk)) 
         {
            // leave this physical page for summary page and seal the block
   ...
   // when the control comes here, 'ppage' contains the next page that can be assigned to a logical page.find the index of the phy page within the block
      ...
      //  g->pg2elem[pgnum_in_gang].e = elem_number;  //changed by menliang

       // mark this block as not free and its state as 'in use'. note that a block could be not free and its state be 'sealed'.
       // it is enough if we set it once while working on the first phy page. also increment the block sequence number.
        if (pp_index == 0) {
    ...
    //////////////////////////////////////////////////////////////////////////////
    // mark the block that corresponds to the active page as not free and 'in_use'.
    switch(currdisk->params.copy_back) {
        case SSD_COPY_BACK_DISABLE:                        // this branch selected
...
void ssd_initialize (void)
{
     ...
     if (disksim->ssdinfo == NULL) 
      {
        ssd_initialize_diskinfo ();
          ...
          ssd_setcallbacks();
          fprintf(stderr, "MAXDEVICES = %d, numssds %d\n", MAXDEVICES, numssds);
...

  {
        ...
        // initialize the cache_updating_in_progress
        currdisk->cache_updating_in_progress = FALSE;
	    ...
	    elem->cache_updating_time = 0.0;
            ...
            // vp - initialize the ssd element metadata   FIXME: where to free these data?
