...
int ssd_choose_element(ssd_timing_t *t, unsigned long blkno)
{
    struct my_timing_t *tt = (struct my_timing_t *) t;
    return (blkno/(tt->params->element_stride_pages * tt->params->page_size)) % tt->params->nelements;   // 1, 8 ,8
     // 
}

int ssd_choose_aligned_count(int page_size, unsigned long blkno, int count)
{
    int res = page_size - (blkno % page_size);   //page_size = 8, blkno and count are getting from the tracefile but count reduce 8 every loop.
    if (res > count)
        res = count;
    return res;
}
 ...
 */
double ssd_write_policy_simple(unsigned long blkno, int count, int elem_num, ssd_t *s)
{
    double cost;
    ...
    unsigned long apn = blkno/tt->params->page_size;
    unsigned long pn = ((apn - (apn % (tt->params->element_stride_pages*tt->params->nelements)))/
...
/*
 makes sure that the number of free blocks is equal to the number of free bits in the bitmap block.
 */
...
/*
 just to make sure that the free blocks maintained across various metadata structures are the same.
 */
...
{
    return (sectors_count * (SSD_BYTES_PER_SECTOR) * s->params.chip_xfer_latency);  //512+16bytes
}
...
/*
 ...
 this code assumes that there is a valid active page where the write can go without invoking new cleaning.
 */
...
double _ssd_write_page_osr(ssd_t *s, ssd_element_metadata *metadata, int lpn)
{
    ...
    //fprintf(stderr, "current lpn is %d, active_page is %d, active_block is %d \n", lpn, active_page, active_block);

    if (metadata->lba_table[lpn] != -1)    // see if this logical page no is already mapped.
   {

        // since the lpn is going to be written to a new location, its previous copy is invalid now. therefore reduce the block
        // usage of the previous copy's block.

        unsigned int prev_page = metadata->lba_table[lpn];    //lpn < 58900
        ...
        unsigned int prev_plane = metadata->block_usage[prev_block].plane_num;            // I guess lpn cannot excess export_size in ssd_init.c

        ssd_assert_page_version(prev_page, active_page, metadata, s);         // make sure the version numbers are correct

        if (metadata->block_usage[prev_block].page[pagepos_in_prev_block] != lpn)      // I guess lpn cannot excess export_size in ssd_init.c
	{
            fprintf(stderr, "Error: lpn %d not found in prev block %d pos %d, pre page is %d\n", lpn, prev_block, pagepos_in_prev_block, prev_page);
               fprintf(stderr, "the assert number is %d \n", metadata->block_usage[prev_block].page[pagepos_in_prev_block]);
            ASSERT(0);
        }
	 else 
	{
    ...
    metadata->lba_table[lpn] = active_page;      // add the entry to the lba table
    metadata->block_usage[active_block].page[pagepos_in_block] = lpn;    // increment the usage count on the active block
    ...
    if (metadata->block_usage[active_block].num_valid >= s->params.pages_per_block)      // some sanity checking
     {
    ...
    cost = s->params.page_write_latency;      // add the cost of the write  //0.2

      ssd_total_write_count++; 
      ssd_write_count++;

    // printf("lpn %d active pg %d\n", lpn, active_page);

    metadata->active_page = active_page + 1;       // go to the next free page
    metadata->plane_meta[active_plane].active_page = metadata->active_page;

    //fprintf(stderr, "OSR--next active_page is  %d \n",  metadata->active_page);
    ...
    if (ssd_last_page_in_block(metadata->active_page, s)) 
    {
        ...
        cost += ssd_data_transfer_cost(s, SSD_SECTORS_PER_SUMMARY_PAGE);     // cost of transferring the summary page data       
        cost += s->params.page_write_latency;  // cost of writing the summary page data
                                   // seal the last summary page. since we use the summary page as a metadata, we don't count it as a valid data page.
...
/*
description: this routine finds the next free block inside a ssd element/plane and returns it for writing. this routine can be called
during cleaning and therefore, must be guaranteed to find a free block without invoking any further cleaning (otherwise there will be a
circular dep). we can ensure this by invoking the cleaning algorithm when the num of free blocks is high enough to help in cleaning.
 */
void _ssd_alloc_active_block(int plane_num, int elem_num, ssd_t *s)
{
    ...
    {
        prev_pos = metadata->block_alloc_pos;  //this way selected
    }
    ...
    ASSERT((bitpos >= 0) && (bitpos < s->params.blocks_per_element));
     ...
     if (plane_num != -1)     // check if we found the free bit in the plane we wanted to
     {
     ...
     {
        metadata->block_alloc_pos = (bitpos+1) % s->params.blocks_per_element;  //No plane constraint for the block finding in the element.
     }
    ...
    if (active_block != -1) 
      {
        ...
        metadata->tot_free_blocks --;    // reduce the total number of free blocks
        pm->free_blocks --;

        //assertion
        ssd_assert_free_blocks(s, metadata);

        ssd_set_bit(free_blocks, bitpos);                   // allocate the block
        metadata->block_usage[active_block].state = SSD_BLOCK_INUSE;
        metadata->block_usage[active_block].bsn = metadata->bsn ++;

        pm->active_page = active_block * s->params.pages_per_block;          // start from the first page of the active block
        metadata->active_page = pm->active_page;
    //fprintf(stderr, "ALLOC: next active_page is  %d \n",  metadata->active_page);
...
double ssd_write_one_active_page(unsigned long blkno, int count, int elem_num, ssd_t *s)
{
    double cost = 0;
    double clean_element_cost = 0;
    int cleaning_invoked = 0;
    ssd_element_metadata *metadata;
    unsigned long lpn;
    unsigned long apn = blkno/s->params.page_size;
    metadata = &(s->elements[elem_num].metadata);

   // metadata->page_usage[apn].state = SSD_PAGE_INSSD;  //added by menliang to create selective cache  

    lpn = ssd_logical_pageno(blkno, s);  

      // see if there are any free pages left inside the active block, as per the osr design, the last page is used as a summary page.
     // so if the active_page is already pointing to the summary page,then we need to find another free block to act as active block.

    if (ssd_last_page_in_block(metadata->active_page, s)) 
  {

        // do we need to create more free blocks for future writes?
        if (ssd_start_cleaning(-1, elem_num, s)) // if the free blocks in the plane is less than 5%, ssd will clean one block for future use.
         {  

    //fprintf(stderr, "The last page in the ssd block, the free blocks in the plane is %d.\n",s->elements[elem_num].metadata.tot_free_blocks);          
          //  if (s->params.cleaning_in_background)    // if we're cleaning in the background, this shouldnot get executed
             //   {
             //   exit(1);  //never set cleaning_in_background =1 when using cache policy --menliang
            //    }
            cleaning_invoked = 1;
            clean_element_cost = ssd_clean_element_no_copyback(elem_num, s);
//fprintf(stderr, "cleaning started in Timing.c, cost is %f\n",clean_element_cost);
            cost = cost + clean_element_cost;
          }

           // if we had invoked the cleaning, we must again check if we need an active block before allocating one. this check is
          // needed because the above cleaning procedure might have allocated new active blocks during the process of cleaning,
         // which might still have free pages for writing.

        if (!cleaning_invoked || ssd_last_page_in_block(metadata->active_page, s)) 
	{
            _ssd_alloc_active_block(-1, elem_num, s);  //if the cleaning process does not start, just find another blank block in the plane is ok.
        }
    }


    // issue the write to the current active page
    //fprintf(stderr, "the elem_num is %d\n", elem_num);
 ...
 */
 /*
static void ssd_compute_access_time_one_active_page_read
...
void ssd_compute_access_time(ssd_t *s, int elem_num, ssd_req **reqs, int total)
{
 ...
 if (apn > 1048576 * 8)
 fprintf (stderr, "the apn has excessed the proper range, apn is %d\n", apn);
 if (lpn > 1048576)
 fprintf (stderr, "the lpn has excessed the proper range, lpn is %d\n", lpn); 

  ssd_current_idle_time = simtime; // decide cache updating time in ssd_cache.c
  overwrite_ratios= metadata->page_usage[lpn].write_count / (metadata->page_usage[lpn].write_count + 1);
//there may exist an error because apn has excessed the tot_pages in ssd_init.c. but if we use the lpn, is the elem_num consistant?
//if we use apn, the Postmark tracefile will cause segament fault.
  if (total == 0) 
        return;
...
//###########################################################################################################XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   ...
   {
      if( reqs[0]->is_read )       //flag =1 in iotrace.file    
      {
         ssd_compute_access_time_one_active_page_read(reqs, total, elem_num, s);  
      }                            
      else                 //   if flag =0 in iotrace.file go this way
      {
       
	 if(overwrite_ratios >= SSD_OVERWRITE_THRESHOLD)
          {
          metadata->page_usage[lpn].write_count ++;
          ssd_compute_cache_access_time_one_active_page(reqs, total,elem_num, s);  
         // fprintf(stderr, "write in to the cache! overwrite_ratios is %f\n", overwrite_ratios);
          }
        else
          {   
             metadata->page_usage[lpn].write_count ++;
...
// insert a function into the structure.
